...
class ObjectronFileDataModule(pytorch_lightning.LightningDataModule):
    ...
    def setup(self, stage=None):
        if not self.issetup:
            ...
            self.train_dataset = ObjectronDataset(OBJECTRON_PATH,split="train", transform=train_transform, objectron_pair=self.pairing)
            self.val_dataset = ObjectronDataset(OBJECTRON_PATH, split="valid", transform=eval_transform, objectron_pair=self.pairing)
            self.test_dataset = ObjectronDataset(OBJECTRON_PATH, split="test", transform=eval_transform, objectron_pair=self.pairing)
